(function( $ ) {
	...
	// 1fix demo video
	$('a#demo-video').on('click', function(e) {
		e.preventDefault();
		
		var $this = $(this),
			riyo_demo_video = $this.attr('data-demoVideo'),
			iframe = '<a href="#" class="close-btn js-modal-close"></a><iframe width="{width}" height="{height}" src="{video}" frameborder="0" allowfullscreen=""></iframe>'

		if( riyo_demo_video ) {
			var video_settings = JSON.parse( riyo_demo_video );
			// check if video status is true
			if( typeof undefined !== typeof video_settings.status && video_settings.status ) {
				iframe = iframe.replace( "{video}", video_settings.src );
				iframe = iframe.replace( "{width}", video_settings.width );
				iframe = iframe.replace( "{height}", video_settings.height );
			}
			// if video status is false, display message only
			else if( typeof undefined !== typeof video_settings.status && ! video_settings.status ) {
				iframe = video_settings.src;
			}
			else {
				iframe = 'Something went wrong.';
			}

			$("body").append( appendthis );
			$(".modal-overlay").fadeTo(300, 0.7, function() {
				$('#riyo-popup').find(' > .popup-container').append( iframe ).parent().removeClass('loading').addClass('video-container');
				$(window).resize();
				$('#riyo-popup').fadeIn()
			});
		}
	...
	*/
	...
	function add_marker( $marker, map ) {
		...
		var icon = {
			url: riyo.url + '/images/map-marker.sbg',
		}
